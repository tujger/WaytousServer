...
public class UserRemove extends AbstractAction<RequestWrapper> {
    ...
    public void call(JSONObject json, final RequestWrapper request) {
        ...
        request.processBody(buf -> {
            ...
            Long userNumber = Long.parseLong(jsonOptions.get(Rest.USER_NUMBER).toString());

            Common.getInstance().getDataProcessor().removeUserFromGroup(groupId, userNumber, request::sendResult, jsonError -> request.sendError(500, jsonError));
        }, exception -> {
            Misc.err(UserRemove.this.getClass().getSimpleName(), exception);
